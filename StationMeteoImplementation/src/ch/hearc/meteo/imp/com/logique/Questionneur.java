...
public class Questionneur implements Runnable {
	...
	public Questionneur(final Questionnable_I questionnable) {
		...
		// Tools
		// cette tehnique permet l'utilisation des méthodes set sur
		// meteoServiceOption
		...
		timeManagerTemperature = new TimeManager(new dt_I() {
			...
			@Override
			public long getDt() {
				return questionnable.getMeteoServiceOptions()
						.getTemperatureDT();
			}
		});
	}
	...
	public void run() {
		...
		while (isRunning) {
			...
			if (!isRunning) // si arret demand�, pas n�cessaire d'attendre
			{
				// update
				timeToWaitMS = min(questionnable.getMeteoServiceOptions()
						.getDelayMS(), timeManagerAltitude.getDt().getDt(),
						timeManagerPression.getDt().getDt(),
						timeManagerTemperature.getDt().getDt());

				System.out.println("[Questionneur] : sleeping " + timeToWaitMS
						+ " (ms) from thread : "
